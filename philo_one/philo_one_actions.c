...
/*   Updated: 2020/06/17 20:01:54 by iwillens         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	philo_getfork(t_philosophers *p, char side)
{
	if (side == FORK_RIGHT)
	{
		...
		ph_setaction(p, AC_TAKENRIGHTFORK, get_time(p->game));
	}
	else if (side == FORK_LEFT)
	{
		pthread_mutex_lock(&(p->left_fork->lock));
		ph_setaction(p, AC_TAKENLEFTFORK, get_time(p->game));
	}
}
...
void	philo_eat(t_philosophers *p)
{
		...
		== p->game->number_of_times_each_philosopher_must_eat)
	{
		...
		{
			ph_setaction(p, AC_ALL_EATEN, get_time(p->game));
